#include "T_TabWidget.h"

#include <QHBoxLayout>
#include <QLabel>
#include <QVBoxLayout>

#include "ElaTabWidget.h"
T_TabWidget::T_TabWidget(QWidget* parent)
    : ElaScrollPage(parent)
{
    _tabWidget = new ElaTabWidget(this);
    QLabel* page1 = new QLabel("Device List", this);
    page1->setAlignment(Qt::AlignCenter);
    QFont font = page1->font();
    font.setPixelSize(32);
    page1->setFont(font);
    QLabel* page2 = new QLabel("Signal Strength", this);
    page2->setFont(font);
    page2->setAlignment(Qt::AlignCenter);
    QLabel* page3 = new QLabel("Network Status", this);
    page3->setFont(font);
    page3->setAlignment(Qt::AlignCenter);
    _tabWidget->addTab(page1, QIcon(":/Resource/Image/Cirno.jpg"), "Device List");
    _tabWidget->addTab(page2, "Signal Strength");
    _tabWidget->addTab(page3, "Network Status");
    QWidget* centralWidget = new QWidget(this);
    centralWidget->setWindowTitle("ElaTabWidget");
    QVBoxLayout* centerVLayout = new QVBoxLayout(centralWidget);
    centerVLayout->setContentsMargins(0, 0, 0, 0);
    centerVLayout->addWidget(_tabWidget);

    addCentralWidget(centralWidget, true, true, 0);
}

T_TabWidget::~T_TabWidget()
{
}
